...
const InaktifData = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/api/master-data?status=0');
      if (res.data.success) {
    ...
    { accessorKey: 'negara', header: 'Negara', enableHiding: true },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => toggleAktif(row.original.kds)} className="text-success hover:text-success/80">
  ...
  const handleBulkToggle = async () => {
    ...
    if (!confirm(`Yakin ingin mengaktifkan kembali ${selectedKds.length} santri terpilih secara massal?`)) return;
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Data Santri Inaktif</h1>
          <p className="text-sm text-slate-500">Santri yang sedang cuti atau keluar</p>
        </div>
        <div className="flex gap-2">
          {Object.keys(rowSelection).length > 0 && (
            <button onClick={handleBulkToggle} className="flex items-center gap-2 bg-success text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-success/90 transition-colors">
              <Power size={18} /> Aktifkan Terpilih ({Object.keys(rowSelection).length})
            </button>
          )}
        </div>
      </div>
            ...
            <tbody>
              {loading ? (
                ...
                table.getRowModel().rows.map(row => (
                  <tr key={row.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors opacity-70 bg-slate-50/50">
